...
class Vehicle(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            base_slug = slugify(f"{self.make}-{self.model}-{self.year}-{self.licence_plate}")
            self.slug = base_slug
...
class VehicleSpecs(models.Model):
    ...
    def __str__(self):
        return f"{self.vehicle.make} ({self.vehicle.licence_plate}) - {self.vehicle.owner_agency.agency_name}"
